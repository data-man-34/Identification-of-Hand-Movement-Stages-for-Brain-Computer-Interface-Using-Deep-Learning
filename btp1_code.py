...
class GalData(Dataset):
    ...
    def prep(self, data):
        ...
        for fc in np.linspace(0,1,11)[1:]:
            ...
            F.append(lfilter(b,a, X0))
        F = np.vstack(F).T
        F = np.concatenate((X,F,F**2), axis=1)

        if self.scale is None:
            self.scale = StandardScaler()
            F = self.scale.fit_transform(F)
        else:
            F = self.scale.transform(F)
    ...
    def load(self, **kwargs):
        ...
        self.__dict__.update(kwargs)
        if validate:
            train, trainlabs, _ = self.readfiles('train', range(1,8))
...
class ConvNet(object):
    ...
    def __init__(self, backend, dataset, subj):
        ad = {
            'type': 'adadelta',
            'lr_params': {'rho': 0.95, 'epsilon': 0.000000001}
        }
        self.layers = []
...
if __name__ == '__main__':
    ...
    if validate:
        ...
        columns = ['HandStart', 'FirstDigitTouch', 'BothStaritLoadPhase',
